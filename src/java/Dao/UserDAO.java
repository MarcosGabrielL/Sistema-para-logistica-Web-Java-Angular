...
public class UserDAO {
  ...
  try {
   ps = connection.prepareStatement("INSERT INTO data_gibgas.users "
           ...
           + "permissao,logo,email,razao,cpf) "
           + "VaLUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
   ...
   ps.setString(22,obj_User.getCpf());
 ...
 return flag;
  ...
  public List<User> read(){
        ...
        try {
            ps = con.prepareStatement("SELECT * FROM data_gibgas.users");
            rs = ps.executeQuery();
