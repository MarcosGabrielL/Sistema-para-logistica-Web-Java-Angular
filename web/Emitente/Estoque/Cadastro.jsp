 ...
 <header>
   ...
   <ul>
      ...
      <li><a href="../Estoque/Retornada.jsp" >Retornada</a></li>
        ...
        e.setTime(data365);
        e.add(Calendar.YEAR, +2);
        data365 = e.getTime();
        java.util.Date Data5 = new java.util.Date(data365.getTime());
        String data5 = formatador.format(Data5);
        Date data365Depois = formatador.parse(data5);
        System.out.println("Daqui a 1 Ano: "+data365Depois );%>
                    <label>Validade <span class="required"></span></label>
                        <input data-mask="00/00/0000" type="text" name="validade" class="field-long" placeholder="<%=formatador.format(data365Depois)%>" />
		...
		<ul class="form-style-1">
                    ...
                    <label>Data Entrada <span class="required"></span></label>
                        <input value="<%=formatador.format(data7)%>" readonly style="background: #eff0f1;" type="text" name="data" class="field-long" placeholder="" />
...
function confirmacao() {
    ...
    document.getElementById("gtni").value = "SEM GTIN";
       ...
       <%ProdutoDAO a = new ProdutoDAO();%>
        document.getElementById("cod").value = padDigits(<%=a.idsem()+1%>,6);
}</script>
<!-- Zeros a Esquerda -->
<script>
